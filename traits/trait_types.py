...
from .trait_base import (strx, get_module_name, class_of, SequenceTypes,
                         TypeTypes, ClassTypes, Undefined, TraitsCache,
                         python_version)

from .trait_handlers import (TraitType, TraitInstance, TraitListObject,
                             TraitSetObject, TraitSetEvent, TraitDictObject,
                             TraitDictEvent, ThisClass, items_event,
                             RangeTypes, HandleWeakRef)

from .traits import (Trait, trait_from, _TraitMaker, _InstanceArgs,
                     code_editor, html_editor, password_editor, shell_editor,
...
try:
    ...
    complex_fast_validate = (11, complex, complexfloating, None, float,
                             floating, int, integer)
...
class String(TraitType):
    ...
    def info(self):
        ...
        msg = ''
        if (self.minlen != 0) and (self.maxlen != sys.maxsize):
            msg = ' between %d and %d characters long' % (self.minlen,
                                                          self.maxlen)
...
class BaseRange(TraitType):
    ...
    def float_validate(self, object, name, value):
        ...
        try:
            if (isinstance(value, RangeTypes) and
                ((self._low is None) or (self._exclude_low and
                                         (self._low < value)) or
        ...
        if not isinstance(value, str):
            try:
                ...
                if (low is None) and (high is None):
                    ...
                    if (((low is None) or (self._exclude_low and
                                           (low < new_value)) or
                         ((not self._exclude_low) and (low <= new_value))) and
                        ((high is None) or (self._exclude_high and
                                            (high > new_value)) or
    ...
    def full_info(self, object, name, value):
        ...
        if low is None:
            ...
            return '%s <%s %s' % (self._type_desc, '=' [self._exclude_high:],
                                  high)

        elif high is None:
            return '%s >%s %s' % (self._type_desc, '=' [self._exclude_low:],
                                  low)

        return '%s <%s %s <%s %s' % (low, '=' [self._exclude_low:],
                                     self._type_desc,
...
class CList(List):
    ...
    def full_info(self, object, name, value):
        """ Returns a description of the trait.
        """
        return '%s or %s' % (self.item_trait.full_info(object, name, value),
            ...
            if issubclass(handler.aClass, HasTraits):
                try:
                    ...
                    return TableEditor(
                        ...
                        filters=[
                            RuleFilterTemplate, MenuFilterTemplate,
                            EvalFilterTemplate
                        ],
                        edit_view='',
...
class Dict(TraitType):
    ...
    def full_info(self, object, name, value):
        ...
        return ('a dictionary with keys which are %s and with values which '
                'are %s') % (self.key_trait.full_info(object, name, value),
...
class BaseInstance(BaseClass):
    ...
    def validate(self, object, name, value):
        ...
        from traits.adaptation import api  # import api  
        adapt = api.adapt
        if value is None:
            if self._allow_none:
                return value
    ...
    def get_default_value(self):
        ...
        if dvt < 0:
            ...
            self.default_value = dv = (self.create_default_value, dv.args,
                                       dv.kw)
