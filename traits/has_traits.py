...
from .trait_notifiers import (
    ExtendedTraitChangeNotifyWrapper, FastUITraitChangeNotifyWrapper,
    NewTraitChangeNotifyWrapper, StaticAnyTraitChangeNotifyWrapper,
    StaticTraitChangeNotifyWrapper, TraitChangeNotifyWrapper)
...
if sys.version_info[0] >= 3:
    ...
    def _get_def(class_name, class_dict, bases, method):
        ...
        if ((result is not None) and is_function_type(result) and
...
def is_bound_method_type(method):
    ...
    return isinstance(method,
                      BoundMethodTypes) or is_cython_func_or_method(method)


def is_unbound_method_type(method):
    """ Test if the given input is a Python method or a Cython method. """
    return isinstance(method,
                      UnboundMethodTypes) or is_cython_func_or_method(method)
...
def _get_instance_handlers(class_dict, bases):
    ...
    for base in bases:
        for name, base_arg_lists in list(
                base.__dict__.get(InstanceTraits).items()):
    ...
    def __init__(self, cls, class_name, bases, class_dict, is_category):
        ...
        for name, value in list(class_dict.items()):
            ...
            if (not rc) and isinstance(value, ForwardProperty):
                ...
                value = Property(getter, setter, validate, True, value.handler,
                                 **value.metadata)
            if rc:
                del class_dict[name]
                if name[-1:] != '_':
                    ...
                    if value_type == 'trait':
                        ...
                        if value._listenable is not False:
                            listeners[name] = (
                                'delegate', get_delegate_pattern(name, value))
                    ...
                    prefix_traits[name] = value
                            ...
                            FunctionType) or is_cython_func_or_method(value):
        ...
        for name in list(class_traits.keys()):
            ...
            if ((instance_handler is not None) and (name in instance_traits) or
                ((instance_handler == '_list_items_changed_handler') and
                 (name[-6:] == '_items') and (name[:-6] in instance_traits))):
            ...
            if events is not None:
                ...
                for event in events:
                    handlers.append(
                        _get_def(class_name, class_dict, bases, '_%s_changed' %
                                 event))
                    handlers.append(
                        _get_def(class_name, class_dict, bases, '_%s_fired' %
                                 event))
...
class HasTraits(CHasTraits, metaclass=MetaHasTraits):
    ...
    *name* attributes defined on the Person class. (The **trait_added**
        ...
        result.update(
            dict([(name, dic[name])
                  for name in self.trait_names(
                      type='delegate', transient=False) if name in dic]))
    ...
    def edit_traits(self,
        ...
        return view.ui(context, parent, kind,
                       self.trait_view_elements(), handler, id, scrollable,
                       args)
    ...
    def trait_view(self, name=None, view_element=None):
        ...
        *view_element* is added to the ViewElements object associated with
        ...
        return self.__class__._trait_view(
            name, view_element, self.default_traits_view,
            self.trait_view_elements, self.visible_traits, self)

    def class_trait_view(cls, name=None, view_element=None):
        return cls._trait_view(
            name, view_element, cls.class_default_traits_view,
            cls.class_trait_view_elements, cls.class_visible_traits, None)
    ...
    def configure_traits(self,
        ...
        if edit:
            ...
            if context is None:
                context = self
            rc = toolkit().view_application(context,
                                            self.trait_view(view), kind,
    ...
    *protocols :
    ...
    for protocol in protocols:
        if not issubclass(type(protocol), ABCMeta):
            raise TraitError("All arguments to 'provides' must be "
...
def implements(*interfaces):
    ...
    *interfaces :
    ...
    callback = deprecated(
        ...
        "Use the 'provides' class decorator.")(callback)
