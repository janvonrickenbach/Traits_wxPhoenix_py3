...
class HasDynamicViews(HasTraits):
    ...
    def trait_view(self, name=None, view_element=None):
        ...
        if not isinstance(name, ViewElement):
            ...
            if (view_element is None) and (name is None or len(name) < 1):
                for dname, declaration in list(
                        self._dynamic_view_registry.items()):
    ...
    def _get_dynamic_elements(self, name):
        ...
        elements = [
            ...
            if hasattr(e, order_trait_name) and (getattr(e, order_trait_name)
                                                 is not None)
        ]
    ...
    def _get_dynamic_handlers(self, name, elements):
        ...
        handlers = [
            getattr(e, handler_name) for e in elements
            if hasattr(e, handler_name) and (getattr(e, handler_name) is
                                             not None)
        ]
