    ...
    def as_ctrait(self):
        ...
        if (getter is not None) or (setter is not None):
            ...
            trait.property(getter,
                           _arg_count(getter), setter,
                           _arg_count(setter), validate, n)
...
class TraitHandler(BaseTraitHandler):
    ...
    def validate(self, object, name, value):
        ...
        """
        raise TraitError("The '%s' trait of %s instance has an unknown type. "
    ...
    def info(self):
        if self._low is None:
            ...
            return '%s >%s %s' % (self._type_desc, '=' [self._exclude_low:],
                                  self._low)
        return '%s <%s %s <%s %s' % (self._low, '=' [self._exclude_low:],
                                     self._type_desc,
...
class TraitTuple(TraitHandler):
    ...
    def full_info(self, object, name, value):
        return 'a tuple of the form: (%s)' % (', '.join([
            self._trait_info(type, object, name, value) for type in self.types
        ]))
...
class TraitListObject(list):
    ...
    def __deepcopy__(self, memo):
        ...
        memo[id_self] = result = TraitListObject(
            self.trait, lambda: None, self.name,
    ...
    def len_error(self, len):
        raise TraitError(
            ...
            (self.name, class_of(self.object()),
             self.trait.full_info(self.object(), self.name, Undefined), len,
             's' [len == 1:]))
...
class TraitSetObject(set):
    ...
    def __reduce_ex__(self, protocol=None):
        ...
        """
        return (copyreg._reconstructor, (type(self), set, list(self)),
                self.__getstate__())
...
class TraitDictObject(dict):
    ...
    def __deepcopy__(self, memo):
        ...
        memo[id_self] = result = TraitDictObject(
            self.trait, lambda: None, self.name,
            dict([copy.deepcopy(x, memo) for x in list(self.items())]))
    ...
    def update(self, dic):
        ...
        if len(dic) > 0:
            ...
            if self.name_items is not None:
                added = {}
                changed = {}
                for key, value in list(new_dic.items()):
                    if key in self:
