...
class BaseTraitValue(HasPrivateTraits):
    ...
    def as_ctrait(self, original_trait):
        ...
        if ((trait is not None) and (notifiers is not None) and
            (len(notifiers) > 0)):
    ...
    def _as_ctrait(self, original_trait):
        ...
        metadata = {'type': 'property', '_trait_value': self}
    ...
    def _as_ctrait(self, original_trait):
        ...
        if self.default is not None:
            ...
            if not isinstance(type, TraitType):
                raise TraitError((
                    "The 'type' attribute of a TraitValue "
            ...
            if self.name == '':
                raise TraitError(
                    "You must specify a non-empty string "
